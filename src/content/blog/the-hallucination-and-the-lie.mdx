...
Where the transition probability for a token $x$ at step $t$ given the history $X_{\lt t}$ is computed via the softmax over the model's output logits:
...
When humans communicate, we generally associate confidence with accuracy. We assume that when a model does not know a fact, its token probabilities will be low, reflecting high entropy. The empirical evidence, however, reveals a striking phenomenon: the **Confidence Inversion**. 
